...
def get_all_chips_from_area(area_name,year_from,year_to):
    ...
    chips = dbms.read("GET_CHIPIDS_FROM_AREA", {"_AREA_": area_name,
                                                "_YEAR_FROM_":str(year_from),
                                                "_YEAR_TO_":str(year_to)})['chipid'].tolist()
...
def calculate_lulc_polygon_intersection(area_name,from_year,to_year,chipids):
    ...
    land_use_change_gdf = dbms.read("CALCULATE_LULC_INTERSECTION", {"_AREA_": area_name, "_FROM_YEAR_": str(from_year), "_TO_YEAR_": str(to_year), "_CHIPID_LIST_": chipid_sql_list})
...
def format_for_db(gdf,area,from_year,to_year):
    ...
    """
    DB_cols = ['area','chipid','year_from','year_to','lulc_category_from','lulc_category_to','area_km2','from_category_area_sq_km','percent_change','geom']

    gdf['area']=area
    gdf['year_from']=from_year
    gdf['year_to']=to_year
    gdf = gdf.rename(columns={'land_use_change':'geom',
...
def calculate_lulc_for_country(country_name,years,chunk_size=8):
    ...
    for year_from in tqdm(years[:-1],desc='from year'):
        ...
        for chip_chunk in tqdm(chip_chunks,desc=f'Looping through each chunk consisting of {chunk_size} chips'):

            gdf = calculate_lulc_polygon_intersection(country_name,year_from,year_to,chip_chunk)
