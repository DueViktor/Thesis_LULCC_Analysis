...
import pandas as pd
import folium
from config import LAND_COVER_PALETTE,LAND_COVER_LEGEND,EPSG_MAPPING,LEGEND_TO_PALETTE
...
def get_all_chips_from_area(area_name):
    ...
    chips = dbms.read("GET_ONLY_CHIPIDS_FROM_AREA", {"_AREA_": area_name})['chipid'].tolist()
...
def get_DW_chips(chip_ids,year,DB=DB):
    results = DB.read('GET_ONLY_DW_LANDCOVER',{'_CHIPIDS_':sql_list_strings(chip_ids),
                                '_AREA_':"Denmark",
                                '_START_YEAR_':year,
                                '_END_YEAR_':year},
                                geom_query = True)
...
def verify_DW_crops(year=2016):
    ...
    for chip_chunk in tqdm(chip_chunks,desc = 'Looping through chunks...'):
        ...
        intersecting_areas_DK = intersecting_areas.to_crs(f'EPSG:{EPSG_MAPPING["Denmark"]}')
    ...
    output = aggregated[['name','area']].groupby(['name']).sum().sort_values(by='area',ascending=False)
    output['percent'] = output['area'].apply(lambda x: round(x/output['area'].sum()*100,2))
...
def plots(aggregated,year=2016):
    ...
    folium.GeoJson(intersecting_areas_dissolved['geometry'],style_function=lambda x: {'fillColor': f'{"red"}', 'color': f'{"red"}', 'weight': 2}).add_to(m)
...
def plot_misclassified_croplands_map(gdf,year=2016,save=False):
    ...
    for idx, row in gdf.iterrows():
        ...
        folium.GeoJson(row['geometry'],style_function=lambda x: {'fillColor': f'{edge_color}', 'color': f'{edge_color}', 'weight': 2}).add_to(m)
