...
path.append("..")
from src.data_handlers import read_wind_turbines, radial_polygon_from_point,process_wind_turbines
...
def plot_threshold(merged, wind_turbines, threshold=0.5):
    ...
    merged['calc_area'] = merged['Rotor-diameter (m)'].apply(lambda x: circle_area(x)/10**6)
    wind_turbines['calc_area'] = wind_turbines['Rotor-diameter (m)'].apply(lambda x: circle_area(x)/10**6)

    merged["distance"] = merged["distance"]/10**3

    # Plot 1
    plt.subplot(1, 2, 1)  # 1 row, 2 columns, 1st subplot
    sns.histplot(merged["calc_area"]/(wind_turbines['calc_area'].sum()), bins=100, cumulative=True, stat="density")
    plt.xlabel("Area")
    plt.ylabel("Percent of Area km^2")
    threshold_area_percentage = merged.loc[merged["distance"] < threshold]["calc_area"].sum() / (wind_turbines['calc_area'].sum())
    plt.axhline(y=threshold_area_percentage, color="red", linestyle="--", label=f"Threshold: {threshold}", linewidth=2)
    ...
    threshold_percentage = merged.loc[merged["distance"] < threshold].shape[0] / merged.shape[0]
    plt.axhline(y=threshold_percentage, color="red", linestyle="--", label=f"Threshold: {threshold} km", linewidth=2)
